        ...
        <form action="{{ route('catalog.index') }}" method="GET">
            <nav class="space-y-6">
                <div>
                    ...
                    <ul class="space-y-2 accordion-content active" id="filter-brands">
                        ...
                        <li class="flex items-center gap-3 group cursor-pointer px-2">
                            <input type="radio" name="brand" value="{{ $item }}" {{ request('brand') == $item ? 'checked' : '' }} class="w-4 h-4 rounded border-outline-variant bg-surface-variant text-primary">
                            <label class="text-label-md text-on-surface-variant"> {{ $item }} </label>
                        </li>
                        @endforeach
                    </ul>
                </div>
                <div>
                    ...
                    <div class="accordion-content active px-2 space-y-4" id="filter-price">
                        <div class="space-y-3">
                            <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Harga Minimum" class="w-full bg-surface-container/50 border border-outline-variant/30 rounded-lg py-2 px-3">
                            <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Harga Maksimum" class="w-full bg-surface-container/50 border border-outline-variant/30 rounded-lg py-2 px-3">
                        </div>
                    </div>
                </div>
                <div>
                    ...
                    <ul class="space-y-2 accordion-content px-2" id="filter-ram">
                        ...
                        <li class="flex items-center gap-3">
                            <input type="checkbox" name="ram[]" value="{{ $ram }}" {{ is_array(request('ram')) && in_array($ram,request('ram')) ? 'checked' : '' }}>
                            <label> {{ $ram }} </label>
                        </li>
                        @endforeach
                    </ul>
                ...
                <div>
                    ...
                    <ul class="space-y-2 accordion-content px-2" id="filter-storage">
                        ...
                        <li class="flex items-center gap-3">
                            <input type="checkbox" name="storage[]" value="{{ $storage }}" {{ is_array(request('storage')) && in_array($storage, request('storage')) ? 'checked' : '' }}>
                            <span> {{ $storage }} </span>
                        </li>
                        @endforeach
                    </ul>
            ...
            <div class="mt-auto pt-6 border-t border-white/5 flex flex-col gap-3">
                <div>
                    ...
                    <a href="{{ route('catalog.index') }}" class="w-full block text-center py-3 border border-outline-variant/30 rounded-xl"> Reset Filter </a>
                </div>
            </div>
        </form>
    </aside>
        ...
        <main class="pb-16 w-full space-y-xl">
            ...
            <section class="w-full mt-8 rounded-3xl overflow-hidden relative h-[450px] group border border-secondary-container/10">
                ...
                <div class="relative z-20 h-full flex flex-col justify-center px-8 md:px-16 max-w-2xl space-y-6">
                    ...
                    <h2 class="text-display-sm font-display-sm text-on-surface leading-tight">{{ isset($setting) && $setting->catalog_title ? $setting->catalog_title : 'Galaxy S24 Ultra' }}</h2>
                    <p class="text-body-lg text-on-surface-variant leading-relaxed">{{ isset($setting) && $setting->catalog_description ? $setting->catalog_description : 'Experience the pinnacle of mobile engineering with titanium durability and revolutionary AI performance.' }}</p>
                ...
                <form action="{{ route('catalog.index') }}" method="GET">
                    <div class="glass-panel bg-surface-container-low/40 border border-secondary-container/10 rounded-3xl p-6 flex flex-col md:flex-row items-center gap-6">
                        <div class="relative flex-1 w-full">
                            ...
                            <input name="search" value="{{ request('search') }}" class="w-full bg-surface-container-highest/30 border border-outline-variant/20 rounded-2xl py-4 pl-12 pr-6 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all placeholder:text-on-surface-variant/40" placeholder="Cari model smartphone, merek, atau spesifikasi..." type="text">
                        </div>
            ...
            <section class="scroll-mt-32" id="semua-produk">
                ...
                <div class="grid w-full grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    ...
                    <div class="group relative bg-surface-container-low/40 border border-secondary-container/10 rounded-3xl hover:border-primary/30 transition-all duration-300 glass-panel p-8 md:p-10">
                        <div class="relative rounded-2xl overflow-hidden mb-6 bg-surface-container/50 h-96">
                            <img src="{{ asset('product/'.$product->brand.'/'.$product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-contain p-2 group-hover:scale-110 transition-transform duration-500">
                        </div>
                        <div class="space-y-4">
                            <div class="flex justify-between items-start">
                                <h3 class="text-headline-md font-bold text-on-surface"> {{ $product->name }} </h3>
                            ...
                            <div class="flex items-center gap-2 text-label-sm font-label-sm text-on-surface-variant">
                                <span class="px-2 py-1 bg-surface-variant rounded-md"> {{ $product->brand }} </span>
                                <span class="px-2 py-1 bg-surface-variant rounded-md">{{ optional($product->spec)->ram }} </span>
                                <span class="px-2 py-1 bg-surface-variant rounded-md">{{ optional($product->spec)->storage }}</span>
                            </div>
                            <div class="pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-outline-variant/10">
                                <div class="text-md font-bold text-primary">Rp {{ number_format($product->price,0,',','.') }}</div>
                                <a href="{{ route('catalog.detail',$product->slug) }}" class="px-4 py-2.5 bg-white/5 hover:bg-white/10 rounded-xl transition-all font-label-md border border-outline-variant/20 whitespace-nowrap"> Detail </a>
                            </div>
                        </div>
                    </div>
                    @empty
                ...
                <div class="mt-16 flex items-center justify-center gap-3">
                    ...
                    @else
                    <a href="{{ $products->previousPageUrl() }}" class="w-12 h-12 flex items-center justify-center rounded-xl glass-panel bg-white/5 border border-outline-variant/20 text-on-surface-variant hover:bg-primary hover:text-on-primary transition-all">
                        <span class="material-symbols-outlined"> chevron_left </span>
                    </a>
                    @endif
                    {{-- Nomor Halaman --}}
                    ...
                    <span class="w-12 h-12 flex items-center justify-center rounded-xl bg-primary text-on-primary font-bold"> {{ $page }} </span>
                    @else
                    <a href="{{ $products->url($page) }}" class="w-12 h-12 flex items-center justify-center rounded-xl glass-panel bg-white/5 border border-outline-variant/20 text-on-surface-variant hover:bg-white/10 transition-all"> {{ $page }} </a>
                    @endif
                    @endfor
                    {{-- Tombol Next --}}
                    @if ($products->hasMorePages())
                    <a href="{{ $products->nextPageUrl() }}" class="w-12 h-12 flex items-center justify-center rounded-xl glass-panel bg-white/5 border border-outline-variant/20 text-on-surface-variant hover:bg-primary hover:text-on-primary transition-all">
