...
<main class="max-w-container-max-width mx-auto px-margin-mobile md:px-margin-desktop pt-40">
    ...
    <section class="relative min-h-[70vh] flex flex-col items-center justify-center text-center overflow-hidden mb-margin-desktop">
        ...
        <div class="max-w-4xl space-y-8 relative z-10">
            ...
            <span class="font-label-caps text-label-caps text-primary uppercase tracking-[0.4em] inline-block border-b border-primary/30 pb-2">
                {{ $setting->hero_label ?? 'PREMIUM SMARTPHONE COLLECTION' }}
            </span>

            <h1 class="font-h1 text-h1-mobile md:text-h1 text-on-surface leading-tight">
                {{ $setting->hero_title ?? 'Temukan Smartphone Impian Anda' }}
            </h1>

            <p class="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto leading-relaxed">
                {{ $setting->hero_description ?? 'Prestige Mobile menghadirkan berbagai pilihan smartphone original dari brand terkemuka dengan garansi resmi, harga kompetitif, dan pelayanan terbaik.' }}
            </p>
        ...
        <div class="mt-16 flex justify-center items-center">
            ...
            <div class="mt-12 flex justify-center items-center">
               ...
               duration-700">
                    ...
                    <img
                        src="{{ isset($setting) && $setting->hero_image ? asset('storage/' . $setting->hero_image) : asset('Images/no-image.png') }}"
                        alt="{{ $setting->hero_title ?? 'Hero Image' }}"
                        class="
                w-auto
                h-auto
    ...
    <section class="mb-margin-desktop grid md:grid-cols-2 gap-gutter items-stretch">
        ...
        <div class="p-gutter md:p-16 space-y-8 glass-panel rounded-3xl flex flex-col justify-center border-l-0 md:border-l border-white/10">
            <h2 class="font-h2 text-h2 text-on-surface leading-tight">
                {{ $setting->vision ?? 'Menjadi toko smartphone terpercaya dan terdepan dalam menyediakan perangkat teknologi berkualitas bagi masyarakat Indonesia.' }}
            </h2>
            <p class="font-body-md text-on-surface-variant leading-relaxed opacity-90">
                {{ $setting->about_store ?? 'ASIAPHONE adalah pusat penjualan smartphone yang menyediakan berbagai pilihan perangkat terbaru dari Samsung, Apple, Xiaomi, Oppo, Vivo, Realme, ASUS ROG, Tecno, Infinix, dan berbagai brand resmi lainnya.' }}
            </p>
            <ul class="space-y-6">
    ...
    <section class="mb-margin-desktop scroll-mt-32" id="katalog">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
            <div>
                ...
                <h2 class="font-h2 text-h2 text-on-surface">{{ $setting->catalog_title ?? 'Katalog Produk' }}</h2>
                
                {{-- ✅ FIX: Filter brand buttons --}}
                <div class="flex flex-wrap gap-3 mt-8">
                    
                    {{-- Tombol "Semua" - Highlight jika TIDAK ada brand dipilih --}}
                    <a href="{{ route('home') }}"
                        class="px-5 py-2 rounded-full {{ empty($brand) ? 'bg-primary text-white' : 'bg-white/5 text-on-surface' }}">
                        Semua
                    </a>
                    
                    {{-- Tombol brand - Loop dengan benar --}}
                    @foreach($brands as $item)
                        <a href="{{ route('home', ['brand' => $item]) }}"
                            class="px-5 py-2 rounded-full {{ $brand == $item ? 'bg-primary text-white' : 'bg-white/5 text-on-surface' }}">
                            {{ $item }}
                        </a>
                    @endforeach
                ...
                <div class="glass-panel rounded-2xl overflow-hidden group">
                    <div class="aspect-[4/3] overflow-hidden relative">
                        <img class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                            src="{{ asset('product/'.strtolower($product->brand).'/'.$product->image) }}"
                            alt="{{ $product->name }}" />
                    </div>
                    <div class="p-6 space-y-4">
                        <h3 class="font-h3 text-xl text-on-surface">{{ $product->name }}</h3>
                        ...
                        <a href="{{ route('catalog.detail', $product->slug) }}" class="block py-3 bg-white/5 border border-white/10 text-on-surface rounded-xl hover:bg-primary hover:text-on-primary transition-all font-semibold text-center">
                            Detail
                        </a>
                    </div>
                </div>
    ...
    <section class="mb-margin-desktop scroll-mt-32" id="hot-deal">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-gutter">
            @forelse($hotDealProducts as $product)
                <div class="glass-panel rounded-2xl overflow-hidden group relative">
                    <div class="absolute top-4 left-4 z-10 bg-error text-on-error px-3 py-1 rounded-full text-label-caps">-{{ $product->discount_percent }}%</div>
                    ...
                    <div class="p-4 space-y-1">
                        <h3 class="font-h3 text-base text-on-surface truncate">{{ $product->name }}</h3>
                        <span class="px-2 py-1 bg-surface-variant rounded-md">{{ optional($product->spec)->ram }} </span>
                        <span class="px-2 py-1 bg-surface-variant rounded-md">{{ optional($product->spec)->storage }}</span>
                        <div class="flex flex-col">
                            <span class="text-lg font-bold text-primary">Rp {{ number_format($product->discount_price,0,',','.') }}</span>
                            <span class="text-xs text-on-surface-variant line-through">Rp {{ number_format($product->price,0,',','.') }}</span>
                        </div>
                        <a href="{{ route('catalog.detail', $product->slug) }}" class="block py-3 bg-white/5 border border-white/10 text-on-surface rounded-xl hover:bg-primary hover:text-on-primary transition-all font-semibold text-center">
                            Detail
                        </a>
            ...
            @forelse($latestProducts as $product)
                <div class="glass-panel rounded-2xl overflow-hidden group">
                    ...
                    <div class="p-4 space-y-3">
                        <h3 class="font-h3 text-lg text-on-surface truncate">{{ $product->name }}</h3>
                        <span class="px-2 py-1 bg-surface-variant rounded-md">{{ optional($product->spec)->ram }} </span>
                        <span class="px-2 py-1 bg-surface-variant rounded-md">{{ optional($product->spec)->storage }}</span>
