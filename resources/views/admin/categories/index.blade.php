        ...
        <div class="glass rounded-2xl p-6 neon-border">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="font-orbitron text-3xl font-bold text-white mt-2">{{ $categories->count() }}</h3>
                </div>
        ...
        <div class="glass rounded-2xl p-6 neon-border">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="font-orbitron text-3xl font-bold text-white mt-2">{{ $categories->where('is_active', 'Y')->count() }}</h3>
                </div>
        ...
        <div class="glass rounded-2xl p-6 neon-border">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="font-orbitron text-3xl font-bold text-white mt-2">{{ $categories->where('is_active', 'N')->count() }}</h3>
                </div>
                    ...
                    <tr class="group hover:bg-cyan-500/5 transition-all">
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-4">
                                ...
                                @endif
                                <div>
                                    <h4 class="text-sm font-bold text-white font-rajdhani tracking-wide">{{ $category->name }}</h4>
                                    @if($category->description)
                                        <p class="text-xs text-gray-400 truncate max-w-xs">{{ Str::limit($category->description, 50) }}</p>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="py-4 px-6">
                            <code class="text-xs text-purple-400 bg-purple-500/10 px-2 py-1 rounded">{{ $category->slug }}</code>
                        </td>
                        <td class="py-4 px-6 text-center">
                            <span class="text-sm font-bold text-white font-orbitron">{{ $category->sort_order }}</span>
                        </td>
                        <td class="py-4 px-6 text-center">
                        ...
                        <td class="py-4 px-6 text-center">
                            <span class="text-sm font-bold text-cyan-400 font-orbitron">{{ $category->products_count ?? 0 }}</span>
                        </td>
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('admin.categories.destroy', $category) }}" 
                                      method="POST" 
                                      class="inline"
                                      onsubmit="return confirm('Apakah Anda yakin ingin menghapus brand \'{{ $category->name }}\'?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
